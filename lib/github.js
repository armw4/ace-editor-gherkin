...
exports.createFeatureFile = async (issueKey, data) => {
  const url = FILE_BASE_URL + featureFilePath(issueKey)
  const body = {
...
exports.updateFeatureFile = async (issueKey, data) => {
  const { sha } = await getFeatureFile(issueKey);
  const url = FILE_BASE_URL + featureFilePath(issueKey);
  const body = {
