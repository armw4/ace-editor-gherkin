...
const FILE_BASE_URL = 'https://api.github.com/repos/armw4/editor-demo/contents';
...
featureFilePath = (issueKey) => '/features/' + issueKey + '.feature'

exports.featureFileExists = async (issueKey) => {
  try {
    await this.getFeatureFile(issueKey);
    return true;
  } catch(e) {
    if (e instanceof ClientError && e.response.status === 404) {
      return false;
    }
...
exports.getFeatureFile = async (issueKey) => {
  const url = FILE_BASE_URL + featureFilePath(issueKey)
